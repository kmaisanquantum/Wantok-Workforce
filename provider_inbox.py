...
content = content.replace('export default function App() {', inbox_component + '\nexport default function App() {')

# 2. Update ProviderNavigationShell to include Inbox
content = content.replace(
    '{ key: "profile", label: "Account", icon: "👤" },',
    '{ key: "profile", label: "Account", icon: "👤" },\n            { key: "messages", label: "Inbox", icon: "💬" },'
)

# 3. Add case "messages" to renderScreen
content = content.replace(
    'case "profile":',
